...
function which_node_gyp() {
  let node_gyp_bin;
  // DEBUG: remove before merging. Child stderr is swallowed by the tests, so append to a file.
  if (process.env.NPG_TRACE_FILE) {
    fs.appendFileSync(process.env.NPG_TRACE_FILE,
      '[TRACE child] npm_config_node_gyp=' + process.env.npm_config_node_gyp +
      ' | exists=' + (process.env.npm_config_node_gyp ? existsSync(process.env.npm_config_node_gyp) : 'n/a') +
      ' | cwd=' + process.cwd() + '\n');
  }
